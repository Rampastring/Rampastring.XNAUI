...
public static class RMath
{
    ...
    public static float AngleFromVector(Vector2 vector) => (float)Math.Atan2(vector.Y, vector.X);
    ...
    public static Vector2 VectorFromLengthAndAngle(float length, float angle) => new(length * (float)Math.Cos(angle), length * (float)Math.Sin(angle));
