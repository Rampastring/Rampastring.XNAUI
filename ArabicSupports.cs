...
namespace Rampastring.XNAUI
{
    ...
    internal class ArabicFixerTool
    {
        ...
        internal static void RemoveTashkeel(ref string str, out List<TashkeelLocation> tashkeelLocation)
        {
            ...
            {
                if (str[i] == (char)0x064B || str[i] == (char)0x064C || str[i] == (char)0x064D)
                {
                    // Tanween Fatha
                    tashkeelLocation.Add(new TashkeelLocation(str[i], i));
                    index++;
                ...
                else if (str[i] == (char)0xFC60 || str[i] == (char)0xFC62 || str[i] == (char)0xFC61)
                {
                    IncrementSB(ref str, i);
                }
            }
        ...
        {
            RemoveTashkeel(ref str, out var tashkeelLocation); char[] lettersOrigin = new char[str.Length]; char[] lettersFinal = str.ToCharArray();
            ...
            for (int i = 0; i < lettersOrigin.Length; i++)
            {
                ...
                if (lettersOrigin[i] == (char)IsolatedArabicLetters.Lam && i < lettersOrigin.Length - 1)
                {
                    switch (lettersOrigin[i + 1])
                    {
                        case (char)IsolatedArabicLetters.AlefMaksoor:
                            ...
                            lettersFinal[i + 1] = (char)0xFFFF; skip = true;
                            break;
                ...
                if (skip) i++;
                if (useHinduNumbers) lettersFinal[i] = (char)HandleInduNumber(lettersOrigin[i], lettersFinal[i]);
            }
            if (showTashkeel && tashkeelLocation.Count > 0)
                ReturnTashkeel(ref lettersFinal, tashkeelLocation);
            internalStringBuilder.Clear(); internalStringBuilder.EnsureCapacity(lettersFinal.Length);
            List<char> ltrChunk = new List<char>();
            bool IsArabic(char c) => (c >= 0x0600 && c <= 0x06FF) || (c >= 0x0750 && c <= 0x077F) || (c >= 0x08A0 && c <= 0x08FF) || (c >= 0xFB50 && c <= 0xFDFF) || (c >= 0xFE70 && c <= 0xFEFF);
            bool IsLTRChar(char c) => char.IsLetterOrDigit(c) || char.IsSymbol(c) || c == ' ' || char.IsPunctuation(c);
            void FlushLTR()
            {
                if (ltrChunk.Count > 0)
                {
                    ltrChunk.Reverse();
                    foreach (var ch in ltrChunk) internalStringBuilder.Append(ch);
            ...
            {
                char c = lettersFinal[i]; if (IsLTRChar(c) && !IsArabic(c)) { ltrChunk.Add(c); }
                else
                {
                    FlushLTR();
                    if ((c == '(' || c == ')' || c == '<' || c == '>' || c == '[' || c == ']') && i > 0 && i < lettersFinal.Length - 1 && IsArabic(lettersFinal[i - 1]) && IsArabic(lettersFinal[i + 1]))
                    {
                        switch (c)
                        {
                            case '(':
                    ...
                    {
                        internalStringBuilder.Append(c); // append Arabic letters as-is
                    }
                }
            }
            FlushLTR();
